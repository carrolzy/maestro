...
TOOL_SPECS: list[JsonDict] = [
    ...
    _tool_schema(
        "route_task",
        "Route Task",
        "Classify a task into Maestro governance tier L0-L3 after read-only preflight signals. Policy layers and runtime facts may only raise the minimum tier.",
        {
            "project": {"type": "string", "description": "Registered project slug."},
            "requirement": {"type": "string", "description": "Task requirement used for validation only; it is never written to the routing log."},
            "repo_root": {"type": "string", "description": "Business repository root used for read-only Git overlap checks."},
            "candidate_files": {"type": "array", "items": {"type": "string"}, "description": "Candidate files identified by read-only preflight."},
            "observed_signals": {"type": "array", "items": {"type": "string"}, "description": "Canonical fast-path and risk signals observed during preflight."},
            "uncertainties": {"type": "array", "items": {"type": "string"}, "description": "Material unresolved facts; any item blocks L0."},
            "requested_actions": {"type": "array", "items": {"type": "string"}, "description": "External, production, destructive, or other requested actions."},
            "current_tier": {"type": "string", "enum": ["L0", "L1", "L2", "L3"], "description": "Existing tier during re-routing; the result cannot be lower."},
            "user_override": {"type": "boolean", "description": "Whether the user explicitly overrode a prior non-hard recommendation."},
        },
        required=["project", "requirement", "repo_root", "candidate_files", "observed_signals", "uncertainties", "requested_actions"],
        output_schema={
            "type": "object",
            "properties": {
                ...
                "hard_vetoes": {"type": "array", "items": {"type": "string"}},
            ...
            "required": ["tier", "confidence", "risk_hits", "hard_vetoes", "required_steps", "skipped_steps", "escalation_triggers", "requires_user_confirmation", "reasons", "warnings"],
            "additionalProperties": False,
        },
    ),
    _tool_schema(
